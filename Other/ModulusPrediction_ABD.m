...
for i = 1 : n
    ...
    % Matrice B
    Bf = (1/2) * Qbar * Distance(i) * St(i);
    B = B + Bf;
    
    % Matrice D
    Df = (1/3) * Qbar * (Distance(i)^2) * St(i);
    D = D + Df;
...
Bm = (1/2) * Qm * distance * (S - St_tot);
B = B + Bm;

Dm = (1/2) * Qm * (distance^2) * (S - St_tot);
D = D + Dm;
...
% figure(1)
% scatter(x,-y,10,z1,'filled')
% colorbar
% colormap(jet(n_div))
% hcb=colorbar;
% set(hcb,'YTick',div)
% axis equal
% axis([0 max(x) round(min(y)) 0])
...
T = table(PositivePly, NegativePly,'RowNames',Rows)
